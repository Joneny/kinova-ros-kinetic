...
bool PickPlace::gripper_action(double finger_turn)
{
    if(robot_connected_ == false)
    {
        if (finger_turn>0.5*FINGER_MAX)
        {
          ...
          std::cout << "finger close with group" << std::endl;
        }
        else
        {
          gripper_group_->setNamedTarget("Open");
          std::cout << "finger open with group" << std::endl;
        }
        gripper_group_->move();
        return true;
    }
...
void PickPlace::define_cartesian_pose()
{
    ...
    start_pose_.pose.position.x = 0.05;
    start_pose_.pose.position.y = -0.388;
    start_pose_.pose.position.z = 0.3;

    q = EulerZYZ_to_Quaternion(0, M_PI/2, 0);
    ...
    grasp_pose_.pose.position.z = 0.5;
...
//    gripper_action(0.8*FINGER_MAX); // partially close
...
//    add_attached_obstacle();
    ...
    ROS_INFO_STREAM("Press any key to send robot back to home position ...");
    std::cin >> pause_;
    group_->clearPathConstraints();
    group_->setNamedTarget("Home");
    evaluate_plan(*group_);*/
